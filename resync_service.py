    ...
    def get_meta_data_records_for_harvester(self, site_dict):
        ...
        try:
            ...
            for site_uuid in site_dict.keys():
                cursor.execute("SELECT m.uuid,m.data,m.changedate,mr.registerdate,mr.rruuid,m.changedate NOT LIKE mr.registerdate AS mchanged FROM metadata m FULL JOIN metadataregistry mr ON m.uuid=mr.gnuuid WHERE m.harvestuuid='"+site_uuid+"' ")
                ...
                for rec in records:
                    ...
                    if rec_rruuid == None:
                        ...
                        gwresponse = self.request_resource_action('resource_registry', 'create', object={"name": rec_name, "description": rec_descrip, "type_": "ExternalDataset"})
                        rruuid = gwresponse[0]
                        ...
                        gwresponse = rruuid = self.request_resource_action('resource_registry', 'create', object={"name": rec_name, "description": rec_descrip, "type_": "ExternalDataset"})
                        rruuid = gwresponse[0]
